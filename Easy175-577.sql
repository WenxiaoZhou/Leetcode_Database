...
with new_activity as (select player_id, event_date as first_login, row_number() over(partition by player_id order by event_date) as first_login_date
from Activity)
...
ON A.player_id=B.player_id and A.event_date=B.Date;
